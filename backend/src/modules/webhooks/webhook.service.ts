...
export class WebhookService {
  ...
  async enqueueWebhookDelivery(merchantId: string, paymentRecord: typeof payments.$inferSelect, eventType: string) {
  ...
  async sendAdHocWebhook(url: string, payload: Record<string, unknown>) {
    try {
      await fetch(url, {
        method: 'POST',
  ...
  async deliverWebhook(deliveryId: string) {
    ...
    const signature = endpoint.secret
      ? signWebhookPayload(delivery.payload as Record<string, unknown>, endpoint.secret)
      : '';
