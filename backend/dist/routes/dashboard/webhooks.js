...
export async function dashboardWebhookRoutes(fastify) {
    ...
    fastify.delete('/webhooks/endpoints/:id', async (request, reply) => {
    ...
    fastify.post('/webhooks/:id/retry', async (request, reply) => {
        const { id } = paramIdSchema.parse(request.params);
        const merchant = request.merchant;
