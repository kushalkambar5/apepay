...
export class PaymentService {
    ...
    async getPaymentByPublicId(publicPaymentId, merchantId) {
    ...
    async getPaymentWithTimeline(publicPaymentId, merchantId) {
    ...
    async getCheckoutSession(publicPaymentId) {
        const [payment] = await db
            .select()
            .from(payments)
    ...
    async markAsPaid(publicPaymentId, txHash) {
        const [payment] = await db
            .select()
            .from(payments)
            .where(eq(payments.paymentId, publicPaymentId))
    ...
    async cancelPayment(publicPaymentId, merchantId) {
        const [payment] = await db
            .select()
            .from(payments)
    ...
    async listMerchantPayments(merchantId, limit = 50, offset = 0) {
        return db
            .select()
            .from(payments)
